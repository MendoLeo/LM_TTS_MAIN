...
norm_conf["mon"] = norm_conf["*"].copy()
# add soft hyphen to punc list to match with fleurs
norm_conf["mon"]["del_set"] += r"\u00AD"

norm_conf["khk"] = norm_conf["mon"].copy()

#=============== Hebrew ===============#

norm_conf["heb"] = norm_conf["*"].copy()
# add "HEBREW POINT" symbols to match with fleurs
norm_conf["heb"]["del_set"] += r"\u05B0-\u05BF\u05C0-\u05CF"

#=============== Thai ===============#

norm_conf["tha"] = norm_conf["*"].copy()
# add "Zero width joiner" symbols to match with fleurs
norm_conf["tha"]["punc_set"] += r"\u200D"

#=============== Arabic ===============#
norm_conf["ara"] = norm_conf["*"].copy()
norm_conf["ara"]["mapping"]["ٱ"] = "ا"
norm_conf["arb"] = norm_conf["ara"].copy()

#=============== Javanese ===============#
norm_conf["jav"] = norm_conf["*"].copy()
norm_conf["jav"]["rm_diacritics"] = True
